...
import csv
disease_suggestions = {}
with open('../diseaseSuggestion1.csv', newline='', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        ...
        suggestions = {
            ...
            'suggestion4': row['Suggestion4']
        }
        disease_suggestions[disease.lower()] = suggestions
        
doctors = {}
with open('../DoctorSuggestion1.csv', newline='', encoding='utf-8') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        doctor_name = row['doctor name']
        doctor_location = row['doctor location']
        specialization = row['specialization']
        contact = row['doctor contact']
...
def MakePredict(request):
	...
	patient_region = Profile.objects.filter(user_id=id).values_list('region', flat=True)
...
'paralysis (brain hemorrhage)':13,'jaudice':14,'malaria':15,'chicken pox':16,'dengue':17,'typhoid':18,'hepatitis a':19,
...
'impetigo':40}
	...
	suggestions = disease_suggestions.get(final_result.lower(), {})
	...
	for doctor, details in doctors.items():
		if final_result.lower() in details['diseases'] and details['location'].lower() == patient_region[0].lower():
			matching_doctors.append({
                'doctor_name': doctor,
                'specialization': details['specialization'],
                'contact': details['contact']
            })
	print('Matching Doctors', matching_doctors)
	return JsonResponse({'status':final_result, 'suggestions': suggestions, 'doctors': matching_doctors})
...
def ehr_upload(request):
    if request.method == 'POST' and request.FILES.get('image'):
        ...
        file_path = os.path.join(settings.BASE_DIR, 'core', 'static', 'core', 'prescription', f'{patient_name}.png')
        ...
        remedies = solve(image_text)

        # Return the result as JSON response
        return JsonResponse({'remedies': remedies})
    else:
        return render(request, 'patient/ehr_upload.html')
...
def solve(text_received):
    image_text = text_received
    print(image_text)
    cohere_api_key = os.getenv('COHERE_API_KEY')
    inp = f"assume you are a doctor, consider the above text and suggest quick remedies for the health problems. strictly only give the quick remedies to help. return each remedy in the form of a string in an array. The text is {image_text}"
    llm = Cohere(cohere_api_key, temperature=0.7)  
    # print(llm.predict(inp))
    remedies = llm.predict(inp)

    # Split the remedies string into an array using newline character '\n' as the delimiter
    remedies_array = remedies.split('\n')
    return remedies_array
